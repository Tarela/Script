    ...
    for line in fp.readlines():
       ...
       #     rev_n_seq = rev(n_seq)
            ...
            if 'N' not in n_seq  :
               ...
               # except:
               #     seq_nmer_dict[ rev_n_seq ]  = 1
    return cut_nmer_dict #seq_nmer_dict, cut_nmer_dict
...
def build_nmer_model(fp, w_plus,w_minus,out,single_nmer_cut,sequence,lflank,rflank,offset):
    ...
    # count  
    #seq_nmer_dict, cut_nmer_dict = count_cut_nmers( fp, w_plus,w_minus, lflank,rflank ,single_nmer_cut,sequence,offset)
    cut_nmer_dict = count_cut_nmers( fp, w_plus,w_minus, lflank,rflank ,single_nmer_cut,sequence,offset)
    #  0 : seq no cut
    ...
    for elem in sorted(dall.keys()):
        if not cut_nmer_dict.has_key(elem):
            ...
            pcut = cut_nmer_dict[elem]
            #pseq = seq_nmer_dict[elem]
            #pbias = pcut*1.0/pseq
        newll = [elem,pcut]#[elem, pbias, pcut, pseq]
...
#========major options=============
    ...
    optparser.add_option("--sc",dest="single_nmer_cut",type="int",default = 20,
                         help="cutoff for cleavage count at single loci , if there's more than N cut at a single loci , we think its caused by amplification ,and reduce to cutoff value,default = 20, 0 means no cutoff")
    optparser.add_option("--left",dest="left",type="int",default = 4,
                         help="region length(left) for calculate seqbias , default =3  nmer = left+right")
    optparser.add_option("--right",dest="right",type="int",default = 4,
