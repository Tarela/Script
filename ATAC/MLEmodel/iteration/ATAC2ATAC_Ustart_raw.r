...
for(iter_num in 1:maxit){
    ...
    this_U <- c()
    for(i in 1:m){
        Ui = Amat_sumj[i] / sum(this_V * Lmat[i,])
...
write.table(summary_V,file=paste0(tissue,"_mergePeak_flank",flankN,"_ATAC2ATAC_Ustart_Vmat.txt"),row.names=T,col.names=T,quote=F,sep="\t")
write.table(summary_U,file=paste0(tissue,"_mergePeak_flank",flankN,"_ATAC2ATAC_Ustart_Umat.txt"),row.names=T,col.names=T,quote=F,sep="\t")
