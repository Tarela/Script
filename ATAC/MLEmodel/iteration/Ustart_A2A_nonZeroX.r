...
ATAC_raw <- read.table(paste0("/nm/vol190/zanglab/sh8tv/Project/scATAC/Data/ChIP_DNase_ATAC_sametissue/ATAC_DNase_pairCMP/kmer_readscount_seqcount_new/",tissue,"_mergePeak_flank",flankN,"_ATAC_readscount_seqtype.bed"),header=T)
DNase_raw <- read.table(paste0("/nm/vol190/zanglab/sh8tv/Project/scATAC/Data/ChIP_DNase_ATAC_sametissue/ATAC_DNase_pairCMP/kmer_readscount_seqcount_new/",tissue,"_mergePeak_flank",flankN,"_DNase_readscount_seqtype.bed"),header=T)
...
ATAC_mat <- ATAC_raw[,4:ncol(ATAC_raw)]#[1:1000,]
DNase_mat <- DNase_raw[,4:ncol(ATAC_raw)]#[1:1000,]
seqtype_mat <- seqtype_raw[,4:ncol(seqtype_raw)]#[1:1000,]
...
for(iter_num in 1:maxit){
    ...
    for(j in 1:n){
        ...
        if(length(idx) == 0){
            ...
            Vj = Amat_sumi[j] / sum(this_U[idx] * Lmat[idx,j])
        }
    ...
    for(i in 1:m){
        ...
        if(length(idx) == 0){
            ...
            Ui = Amat_sumj[i] / sum(this_V[idx] * Lmat[i,idx])
        }
...
write.table(summary_V,file=paste0(tissue,"_mergePeak_flank",flankN,"_Ustart_A2A_nonZeroX_Vmat.txt"),row.names=T,col.names=T,quote=F,sep="\t")
write.table(summary_U,file=paste0(tissue,"_mergePeak_flank",flankN,"_Ustart_A2A_nonZeroX_Umat.txt"),row.names=T,col.names=T,quote=F,sep="\t")
