...
#      if(U_new[i] >= 0 && U_new[i] <= sqrt(bound)){
      if(U_new[i] >= 0 && U_new[i] <= 2){
        diff = sum(X[i, ] * I[i, ] * (log(U_new[i]) - log(U[i]))) - temp_U * sum(V * L[i, ] * I[i, ])
        if(runif(1) < exp(diff)){
    ...
    for(j in 1:length(V)){
      ...
      if(V_new[j] >= 0 && V_new[j] <= sqrt(bound)){
        diff = sum(X[, j] * I[, j] * (log(V_new[j]) - log(V[j]))) - temp_V * sum(U * L[, j] * I[, j])
        if(runif(1) < exp(diff)){
    ...
    pii_record[k] = pii
    print(c(round(k), round(pii,3), (proc.time() - ptm)[3]))
    ptm <- proc.time()
  }
...
write.table(round(U,4),file=paste(tissue,flankN,datatype,"U_ZIP.txt",sep="_"),row.names=T,col.names=F,sep="\t",quote=F)
write.table(round(V,4),file=paste(tissue,flankN,datatype,"V_ZIP.txt",sep="_"),row.names=F,col.names=F,sep="\t",quote=F)
...
write.table(round(outU,4),file=paste(tissue,flankN,datatype,"outU_ZIP.txt",sep="_"),row.names=T,col.names=F,sep="\t",quote=F)
...
pdf(file=paste(tissue,flankN,datatype,"_ZIP.pdf",sep="_"),height=9,width=9)
