...
vector_generator_simplex = function(X, L, iter){
  ...
  for(m in 2:iter){
    ...
    for(i in 1:length(U)){
      U_new = U
      temp_U = runif(1, -0.05, 0.05) #random walk of U is bounded into [0, 2]
      U_new[i] = U[i] + temp_U #sequentailly update U
      if(U_new[i] >= 0 && U_new[i] <= 2){
        diff = sum(I[i, ] * X[i, ]) * (log(U_new[i]) - log(U[i])) - temp_U * sum(V * L[i, ] * I[i, ])
        if(runif(1) < exp(diff)){
    ...
    for(k in 1:length(B)){
      B_new = B
      temp_B = runif(1, -0.05, 0.05) #random walk of B, which is bounded
      B_new[k] = B[k] + temp_B #sequentailly update B
      if(B_new[k] >= -10 && B_new[k] <= 10){
        V = exp(H %*% B)
        V_new = exp(H %*% B_new)
        diff = sum((X * I) %*% (log(V_new) - log(V))) + sum(((L * I) %*% (V - V_new)) * U)
        if(runif(1) < exp(diff)){
          B[k] = B_new[k] #B is updated
        }
      }
    }
    U_record[m, ] = U
...
write.table(round(U,4),file=paste(tissue,flankN,datatype,"U_simplexZIP.txt",sep="_"),row.names=T,col.names=F,sep="\t",quote=F)
write.table(round(V,4),file=paste(tissue,flankN,datatype,"V_simplexZIP.txt",sep="_"),row.names=F,col.names=F,sep="\t",quote=F)
write.table(round(B,4),file=paste(tissue,flankN,datatype,"B_simplexZIP.txt",sep="_"),row.names=F,col.names=F,sep="\t",quote=F)
write.table(round(PI,4),file=paste(tissue,flankN,datatype,"PI_simplexZIP.txt",sep="_"),row.names=F,col.names=F,sep="\t",quote=F)
outU <- cbind(U[,1],apply(U[,(ncol(U)-100):ncol(U)],1,mean))
rownames(outU) <- rownames(U)
write.table(round(outU,4),file=paste(tissue,flankN,datatype,"outU_simplexZIP.txt",sep="_"),row.names=T,col.names=F,sep="\t",quote=F)
...
pdf(file=paste(tissue,flankN,datatype,"_simplexZIP.pdf",sep="_"),height=9,width=9)
