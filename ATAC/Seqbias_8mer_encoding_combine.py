...
def readBG(cutfile,seqfile):
    ...
    for line in inf2:
        ...
        seqtype = ll[0]
        seqcount = ll[1]
        if cutD.has_key(seqtype):
            if float(seqcount) == 0:
                bias = 0
            else:
                bias = float(cutD[seqtype])/float(seqcount)
        else:
            bias = 0
        outBG[seqtype] = bias                
...
def pred_obs_bias(output,cut,seq):
...
#========major options=============
    optparser.add_option("-o","--output",dest="output",type="str",
                         help="")
    optparser.add_option("-c","--cut",dest="CUT",type="str",
                         help="sequence cleavage matrix at shift 0")
    optparser.add_option("-s","--seq",dest="SEQ",type="str",
                         help="sequence occurance matrix at shift 0")
...
#    bgmatrix = options.bgmatrix
    pred_obs_bias(out_bed,options.CUT,options.SEQ)
