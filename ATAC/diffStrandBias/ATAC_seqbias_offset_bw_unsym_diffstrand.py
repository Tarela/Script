...
def rev(seq):
    ...
    for i in seq[::-1]:
        ...
        revseq += r
    return revseq
def count_cut_nmers( fp, w_plus,w_minus,lflank, rflank ,single_nmer_cutoff,sequence,offset):
    """
    ...
    for line in fp.readlines():
        ...
        #nseq = genome[chrm][(start-lflank-offset):(end+rflank-offset)].upper()
        ...
        for k in range( len(cp) ):
            ...
            loci = start + k
            try:
                p_seq = genome[chrm][(loci + offset - lflank) : (loci + offset + rflank)].upper()#pseq[k:(k+lflank+rflank)]
                n_seq = genome[chrm][(loci - offset+1-rflank) : (loci - offset+1+lflank)].upper()#nseq[(k+1):(k+lflank+rflank+1)]
            except:
                print chrm,loci + offset - lflank,loci + offset + rflank,loci - offset+1-rflank,loci - offset+1+lflank
                continue
       #     rev_n_seq = rev(n_seq)
            if single_nmer_cutoff != 0:
                p_cut = min(p_cut,single_nmer_cutoff)
                n_cut = min(n_cut,single_nmer_cutoff)
                
            if 'N' not in p_seq:
                try: 
                    cut_nmer_dict_p[ p_seq ] += p_cut
                except:
                    cut_nmer_dict_p[ p_seq ]  = p_cut
                try: 
                    seq_nmer_dict_p[ p_seq ] += 1
                except:
                    seq_nmer_dict_p[ p_seq ]  = 1
            if 'N' not in n_seq :
                rev_n_seq = rev(n_seq)
                try:
                    cut_nmer_dict_n[ rev_n_seq ] += n_cut
                except:
                    cut_nmer_dict_n[ rev_n_seq ]  = n_cut
                try: 
                    seq_nmer_dict_n[ rev_n_seq ] += 1
                except:
                    seq_nmer_dict_n[ rev_n_seq ]  = 1
    return seq_nmer_dict_p, cut_nmer_dict_p, seq_nmer_dict_n, cut_nmer_dict_n
...
def build_nmer_model(fp, w_plus,w_minus,out,single_nmer_cut,sequence,lflank,rflank,offset):
    """
    ...
    # count  
    seq_nmer_dict_p, cut_nmer_dict_p, seq_nmer_dict_n, cut_nmer_dict_n = count_cut_nmers( fp, w_plus,w_minus, lflank,rflank ,single_nmer_cut,sequence,offset)
    # -1 : no seq , 0 : seq no cut
    dall_p = make_nmer_dict(nmer)
    dall_n = make_nmer_dict(nmer)
    #print len(allcut_nmer_dict),len(cut_nmer_dict_p),len(cut_nmer_dict_n)
    # normalize
    for elem in seq_nmer_dict_p:
    
        if not dall_p.has_key(elem):
            continue
        try : 
            val = cut_nmer_dict_p[elem]
        except:
            val = 0

        if seq_nmer_dict_p[elem] != 0 : 
            dall_p[elem]  =  val / (1.0*seq_nmer_dict_p[elem]) 
        else:
            dall_p[elem] = 0
 
    for elem in seq_nmer_dict_n:
    
        if not dall_n.has_key(elem):
            continue
        try : 
            val = cut_nmer_dict_n[elem]
        except:
            val = 0

        if seq_nmer_dict_n[elem] != 0 : 
            dall_n[elem]  =  val / (1.0*seq_nmer_dict_n[elem]) 
        else:
            dall_n[elem] = 0
            
    outf = open(out,'w')
    for seqtype in sorted(dall_p.keys()):
        pout = str(dall_p[seqtype])
        nout = str(dall_n[seqtype])
...
#========major options=============
    ...
    optparser.add_option("--offset",dest="offset",type="int",default = 0,
                         help="bp of shift center,default is 0 ")    
    optparser.add_option("--genome",dest="genome",type="str",
                         help="twobit sequence file")
    optparser.add_option("--sc",dest="single_nmer_cut",type="int",default = 20,
                         help="cutoff for cleavage count at single loci , if there's more than N cut at a single loci , we think its caused by amplification ,and reduce to cutoff, 0 means no cutoff")
...
#========minor options=============
    ...
    fp = open(peak)
    build_nmer_model(fp, w_plus,w_minus,out,single_nmer_cut,genome,options.left,options.right,offset)
