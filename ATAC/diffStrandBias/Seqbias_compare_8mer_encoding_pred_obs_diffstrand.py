...
def pred_obs_bias(output,bg0):
    ...
    nb0s0,nb1s0,nb2s0 = paramest(nBG)
    
    #print b0s0,b1s0,b2s0
    outf = open(output,'w')
    for seqtype in sorted(pBG.keys()):
        if pBG[seqtype] > 0 and nBG[seqtype] > 0:            
            obs = numpy.log(pBG[seqtype])
            pred = mypredict(seqtype,b0s0,b1s0,b2s0,8)
            obs2 = numpy.log(nBG[seqtype])
            pred2 = mypredict(seqtype,nb0s0,nb1s0,nb2s0,8)
            newll = [seqtype,obs,pred,obs2,pred2]
