...
def seq2biasParm(useseq,encoding_paramters,code):
    #param_length_b0 = 1
...
def get_regionLevel_simplex_parameters(inputbed,outputbed,plusbw,minusbw,biasmat,genome2bit,ext):
    t0 = time.time()
...
#    seq = genome[chrm][(int(ll[1])-flank):(int(ll[1])+flank)].upper()
    ...
    for line in inf:
        ...
        #plusSequence_reverse = genome[chrm][(start-flank+9):(end+flank+9)].upper()
        #minusSequence_reverse = genome[chrm][(start-flank+1 -9):(end+flank+1-9)].upper()
...
#========major options=============
    ...
    optparser.add_option("-e","--ext",dest="ext",type="int",default=200,
                         help="extend size from the center of input bed region")              
    optparser.add_option("-s","--sequence",dest="sequence",type="str",default='/Data/Genome/hg38/hg38.2bit',
                         help="whole genome sequence in 2bit format")
    optparser.add_option("-b","--biasmat",dest="biasmatrix",type="str",
                         help="matrix of cutting bias, first 3 column is 8mer seq and raw bias and encbias ")
...
#========minor options=============
    ...
    if not inputbed or not outputbed:
        ...
        sys.exit(1)
    get_regionLevel_simplex_parameters(inputbed,outputbed,plusbw,minusbw,biasmat,options.sequence,options.ext)
