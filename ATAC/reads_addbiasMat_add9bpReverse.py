...
def PEreads_feature(inline, GCrange, flank, genome2bit,biasMat):
    ll = inline.split()
    ...
    return ll + [biasP,biasN,CBvalue(biasP,biasP_shift9N),CBvalue(biasN,biasN_shift9P),seqGC]
...
def dupFeature(inputbed, outputfilename, seq2bit, GCrange, flankLen, biasfile):
    ...
    for line in inf:        
        newll = PEreads_feature( line, GCrange, flankLen, genome,bias)
        if not "NA" in newll:
