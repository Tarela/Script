...
def get_regionLevel_sequence(inbed,outputname,species,flank):
    genome = twobitreader.TwoBitFile("/nv/vol190/zanglab/sh8tv/Data/Genome/%s/%s.2bit"%(species,species))
...
#========minor options=============
    ...
    if not inputbed or not outputname:
        ...
        sys.exit(1)
    get_regionLevel_sequence(inputbed,outputname,options.species,options.flank)
