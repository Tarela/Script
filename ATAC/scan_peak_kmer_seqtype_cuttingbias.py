...
def get_regionLevel_sequence(inbed,outbed,species,biasmat):
...
#                print chrm,start,end,i,mcuts,minus_biassum,minus_biasCB
        ...
        newll_seq = ll[:3] + [biasvalue]#+ [Sdict[x] for x in sorted(Sdict.keys())]
...
#                         help="1bp resolution cleavage bigwig for minus strand cuts")
    optparser.add_option("-b","--biasmat",dest="biasmat",type="str",#default=4,
                         help="biasmat")
...
#========minor options=============
    ...
    if not inputbed or not outputname:
        ...
        sys.exit(1)
    get_regionLevel_sequence(inputbed,outputname,options.species,options.biasmat)
