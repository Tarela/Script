...
def get_regionLevel_simplex_parameters(inputbed,outputbed,plusbw,minusbw,ext,genome2bit):
...
#    seq = genome[chrm][(int(ll[1])-flank):(int(ll[1])+flank)].upper()
    ...
    for line in inf:
        ...
        plus_data = numpy.array([0.0]*88)#len(B))
        minus_data = numpy.array([0.0]*88)#len(B))
...
#========minor options=============
    ...
    if not inputbed or not outputbed:
        ...
        sys.exit(1)
    get_regionLevel_simplex_parameters(inputbed,outputbed,plusbw,minusbw,options.ext,options.sequence)
