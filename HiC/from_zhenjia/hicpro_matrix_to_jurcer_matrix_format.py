...
def write_out_juicer_format_matrix(order_index_file,matrix_file,outdir,data,resolution,flag):
    ...
    for chrom in chroms:
        outfile_name = outdir+os.sep+'{}_{}_{}_{}.matrix'.format(data,resolution,flag,chrom)
        print(outfile_name)
...
def main(data_type):
    ...
    #par_outdir = '/nv/vol190/zanglab/zw5j/work2017/T_ALL_CTCF/11_HiC_analysis/f1_preprocess/union_binding_processed_data/panos/transformed_matrix'
    par_indir = '/nv/vol190/zanglab/zw5j/work2017/T_ALL_CTCF/11_HiC_analysis/f1_preprocess/pancancer_HiC_Pro_mboi/pancancer_mboi/hic_results/matrix'
    
    if data_type in ['A549','HCT116','trans_colon1','trans_colon2']:
        for resolution in [5000,10000,  25000]:
            #for normalization in ['raw','iced']:
            if 1:
                outdir = '{}/{}'.format(data_type,resolution)
                ...
                print(data_type,resolution,os.path.isfile(index_file),os.path.isfile(raw_matrix),os.path.isfile(iced_matrix))
    ...
    # hindiii
    par_indir = '/nv/vol190/zanglab/zw5j/work2017/T_ALL_CTCF/11_HiC_analysis/f1_preprocess/pancancer_HiC_Pro_hindiii/pancancer_hindiii/hic_results/matrix'
    if data_type in ['lung']:
        for resolution in [5000, 10000, 25000]:
            if 1:
                outdir = '{}/{}'.format(data_type,resolution)
                os.makedirs(outdir,exist_ok=True)
