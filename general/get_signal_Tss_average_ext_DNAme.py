...
def summary_DNAme_pattern(indata):
    ...
    if filterdata == [''] or len(filterdata) == 0:
        ...
        return numpy.mean(realdata)
    #NCpG = (len(realdata))
    #NmeCpG = (len(realdata[realdata > 0.8]))
    #NunmeCpG = (len(realdata[realdata <= 0.2]))

    #if len(realdata) == 0:
    #    return -1 #aveDNAme.append(-1)
    #else:
    #    return (numpy.mean(realdata))


def get_signal(inputfile,output,signalbw,extend,bwfolder):
...
#========minor options=============
    ...
    get_signal(inputfile,output,signalbw,options.ext,bwfolder)
