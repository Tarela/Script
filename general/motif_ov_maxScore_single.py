...
def add_ov_motif_score(inputpeak,motif_file,outname):
    ### overlap
    inputdict = {}
    inf = open(inputpeak)
    for line in inf:
...
#    allmotif = motif_names.split(",")
    ...
    cmd = 'intersectBed -a %s -b %s -wao > %s'%(inputpeak,motif_file,outname+'.tmp')
    sp(cmd)
    time.sleep(20)
...
#========major options=============
    ...
    optparser.add_option("-m",'--motiffile',dest="motif_file",type="str",default = "",
                         help="motif file, eg: MC001_AR.bed")
#========minor options=============
    ...
    if not inputfile:
        ...
        sys.exit(1)
    add_ov_motif_score(inputfile,options.motif_file,options.output)
