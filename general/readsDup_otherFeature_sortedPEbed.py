...
def PEreads_feature(ll, GCrange, flank, genome2bit):
    ...
    return ll + [seqP,seqN,seqGC,fraglen]
...
def dupFeature(inputbed, outputfilename, seq2bit, GCrange, flankLen):
    ...
    for line in inf:        
        if last_reads == "NA" or judge_same_read(last_reads, line):
            ...
            newll = PEreads_feature( ll_last[:3]+[read_name,now_dup_num], GCrange, flankLen, genome)
