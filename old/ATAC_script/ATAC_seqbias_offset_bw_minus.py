...
def rev(seq):
    ...
    for i in seq[::-1]:
        ...
        revseq += r
    return revseq
def count_cut_nmers( fp,w_minus,lflank, rflank ,single_nmer_cutoff,sequence,offset):
    """
...
def build_nmer_model(fp,w_minus,out,single_nmer_cut,sequence,lflank,rflank,offset):
    """
    ...
    # count  
    seq_nmer_dict, cut_nmer_dict = count_cut_nmers( fp, w_minus, lflank,rflank ,single_nmer_cut,sequence,offset)
...
#    w_plus = options.wp
    ...
    fp = open(peak)
    build_nmer_model(fp, w_minus,out,single_nmer_cut,genome,options.left,options.right,offset)
