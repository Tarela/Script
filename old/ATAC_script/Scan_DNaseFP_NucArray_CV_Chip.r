...
outfile <- paste0(a[2],'mnase_cv_chipbw_10k.pdf')
...
each<-(ncol(data)-6-200)/6
#print(each)
TC <-  apply(data[,(206+5*each+1):(206+6*each)],1,mean)
#length(which(TC > 0.5))
#hist(TC*50,nclass=200,xlim=c(0,50))
#plot(density(TC))
mnase <- data[order(TC),(6+1):(206)]
pc <- data[order(TC),(206+1):(206+each)]
nc <- data[order(TC),(206+each+1):(206+each+each)]
pb <- data[order(TC),(206+each+each+1):(206+each+each+each)]
nb <- data[order(TC),(206+3*each+1):(206+4*each)]
cv <- data[order(TC),(206+4*each+1):(206+5*each)]
chip <- data[order(TC),(206+5*each+1):(206+6*each)]
...
layout(rbind(c(1,1,2,2,3,3,4,5),c(6,6,8,10,12,14,16,18),c(7,7,9,11,13,15,17,19)),heights=c(1,2,0.3))
