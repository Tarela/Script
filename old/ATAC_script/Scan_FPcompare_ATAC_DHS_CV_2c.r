...
#plot(density(TC))
pac <- data[order(TC),(6+1):(6+each)]
nac <- data[order(TC),(6+each+1):(6+each*2)]
pab <- data[order(TC),(6+2*each+1):(6+3*each)] 
nab  <- data[order(TC),(6+3*each+1):(6+4*each)]
pdc <- data[order(TC),(6+4*each+1):(6+5*each)]
ndc <- data[order(TC),(6+5*each+1):(6+6*each)]
pdb <- data[order(TC),(6+6*each+1):(6+7*each)]
ndb <- data[order(TC),(6+7*each+1):(6+8*each)]
...
cmp_rg <- (floor(ncol(pac)/2)-49 ):(floor(ncol(pac)/2)+50 ) 
...
layout(rbind(c(1,1,2,2,3,3,4,4,5),c(6,8,10,12,14,16,18,20,22),c(7,9,11,13,15,17,19,21,23)),heights=c(1,2,0.3))


sitepro(mpac, mnac,c("red","blue"),c("+ATAC","-ATAC"),paste0("ATAC : ",round(c_AP,4)))
sitepro(mpab, mnab,c("red","blue"),c("+ATACbg","-ATACbg"),"ATAC bias")
sitepro(mpdc, mndc,c("red","blue"),c("+DNase","-DNase"),paste0("DNase : ",round(c_DP,4)))
