...
#            nnew.append(pow(numpy.e,(n_yf+n_yr)[0]))
        ...
        code_seq = genome[chrm][(start-offset +bpshift -span):(end+12 - bpshift +span)].upper()
        seq_f = code_seq[:-9]
        seq_r = code_seq[9:]
   ...
   #     print len(seq_f),len(yf)
        ...
        pnew = list(pow(numpy.e,z[8:]))
        nnew = list(pow(numpy.e,z[:-8]))
       # seq_f6 = code_seq[8:-9]
...
#            nnew_assign.append(ntotal * nnew[bp+span]/sum(nnew[bp:(bp+2*span)]))
    ...
    #        nnewlin_assign.append(ntotal * nnew_linear[bp+span]/sum(nnew_linear[bp:(bp+2*span)]))
        ...
        newll = ll + pnew[span:(len(p_sum)-span)] + nnew[span:(len(n_sum)-span)]
