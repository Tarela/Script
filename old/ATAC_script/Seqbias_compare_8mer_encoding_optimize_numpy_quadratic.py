...
def paramest(seqd):
    ...
    for j,seq in enumerate(seqd):
        if seqd[seq] == 0:
            continue
        y.append(numpy.log(seqd[seq]))
        x.append(sum(mypredict(seq,b0,b1,b2,len(seq))))
        
    Y = numpy.array(y)
    x = numpy.array(x)
    x1 = numpy.transpose(numpy.vstack((x,numpy.ones(len(x)))))
    X = numpy.transpose(numpy.vstack((x*x,x,numpy.ones(len(x)))))
    X3 = numpy.transpose(numpy.vstack((x*x*x,x*x,x,numpy.ones(len(x)))))
...
def weight(peak,w_plus,w_minus,bg0,span,gen,lflank,rflank,offset,bpshift,outfile,pseudo):
    nmer = lflank + rflank
    ...
    for line in inf:### chr start end name motifscore strand FP DNase chip
        ...
        p_sum = list(w_plus_H.summarize(chrm,start-span,end+span,end-start+2*span).sum_data + pseudo )
        n_sum = list(w_minus_H.summarize(chrm,start-span,end+span,end-start+2*span).sum_data + pseudo )
...
def sitepro_scan(peak,out_bed,w_plus,w_minus,bg0,span,gen,lflank,rflank,offset,bpshift):
    ...
    code = encoding()
    b0s0,b1s0,b2s0,B1,B2,B3 = paramest(pBG) 
    #b0s0,b1s0,b2s0 = paramest(pBG)
...
#            nnew.append(pow(numpy.e,(n_yf+n_yr)[0]))
   ...
   #     print len(seq_f),len(yf)
        ...
        yf2 = yf*yf*B2[0] + yf * B2[1] + B2[2]
        yr2 = yr*yr*B2[0] + yr * B2[1] + B2[2]
        
        yf3 = yf*yf*yf*B3[0] + yf * yf * B3[1] + yf * B3[2] + B3[3]
        yr3 = yr*yr*yr*B3[0] + yr * yr * B3[1] + yr * B3[2] + B3[3]
        
        z2 = yf2 + yr2
        z3 = yf3 + yr3
        
        pnew2 = list(pow(numpy.e,z2[(offset-1):]))
        nnew2 = list(pow(numpy.e,z2[:(-offset+1)]))
         
        pnew3 = list(pow(numpy.e,z3[(offset-1):]))
        nnew3 = list(pow(numpy.e,z3[:(-offset+1)]))
        praw_assign =[]
        nraw_assign =[]
        px_assign  = []
        nx_assign  = []
        ...
        for bp in range(len(p_sum)- 2*span):
            ...
            pnew2_assign.append(ptotal * pnew2[bp+span]/sum(pnew2[bp:(bp+2*span)]))
            nnew2_assign.append(ntotal * nnew2[bp+span]/sum(nnew2[bp:(bp+2*span)]))
            pnew3_assign.append(ptotal * pnew3[bp+span]/sum(pnew3[bp:(bp+2*span)]))
            nnew3_assign.append(ntotal * nnew3[bp+span]/sum(nnew3[bp:(bp+2*span)]))
...
#        newll = ll +  p_sum[span:(len(p_sum)-span)] + n_sum[span:(len(n_sum)-span)]  + pweight_assign + nweight_assign
        newll = ll + p_sum[span:(len(p_sum)-span)] + n_sum[span:(len(n_sum)-span)] +px_assign +nx_assign+  pnew_assign + nnew_assign +pnew2_assign + nnew2_assign +pnew3_assign + nnew3_assign
...
#    optparser.add_option("-p","--pseudo",dest="pseudo",type="float",default = 0,
#                         help="pseudo add to each bp of training data when predict weight,default is 0 ")
...
#    wgt = weight(train,w_plus,w_minus,bgmatrix,Cspan,gen,lflank,rflank,offset,options.bpshift,out_bed[:-4]+"_weight.txt",options.pseudo)
