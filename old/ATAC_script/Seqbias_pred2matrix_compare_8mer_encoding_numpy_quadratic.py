...
#    dnewlin = make_dict(assign_limit,cut_limit)
    ...
    for line in inf:
        ...
        pnew2  =  ll[(400*6 + 5):(400*7+5)]
        nnew2  =  ll[(400*7 + 5):(400*8+5)]
        pnew3  =  ll[(400*8 + 5):(400*9+5)]
        nnew3  =  ll[(400*9 + 5):(400*10+5)]
 ...
 #       nnewlin = ll[(400*3 + 5):(400*4+5)]
       ...
       #     cut_raw_addup += pow( int(float(pc[i])) - float(praw[i]) ,2)
            ...
            cut_new2_addup += pow( int(float(pc[i])) - float(pnew2[i]) ,2)
            cut_new3_addup += pow( int(float(pc[i])) - float(pnew3[i]) ,2)
        for i in range(400):
       #     cut_raw_addup += pow( int(float(nc[i])) - float(nraw[i]) ,2)        
            ...
            cut_new2_addup += pow( int(float(nc[i])) - float(nnew2[i]) ,2)
            cut_new3_addup += pow( int(float(nc[i])) - float(nnew3[i]) ,2)
    ...
    cut_new2_rmse = pow(cut_new2_addup/(peaknum*800),0.5)
    cut_new3_rmse = pow(cut_new3_addup/(peaknum*800),0.5)
    ...
    outall.write("\t".join(["cut_new_quadratic_rmse",str(cut_new2_rmse)])+"\n")
    outall.write("\t".join(["cut_new_cubic_rmse",str(cut_new3_rmse)])+"\n")
    outall.close()
# ------------------------------------
# Main function
# ------------------------------------
