...
#    dnewlin = make_dict(assign_limit,cut_limit)
    ...
    for line in inf:
        ...
        pmean =   ll[(400*2 + 5):(400*3+5)]
        nmean =   ll[(400*3 + 5):(400*4+5)]
        pprop = ll[(400*4 + 5):(400*5+5)]
        nprop = ll[(400*5 + 5):(400*6+5)]
        pequal = ll[(400*6 + 5):(400*7+5)]
        nequal = ll[(400*7 + 5):(400*8+5)]
        #pmean  =  ll[(400*2 + 5):(400*3+5)]
        #nmean  =  ll[(400*3 + 5):(400*4+5)]
 ...
 #       nnewlin = ll[(400*3 + 5):(400*4+5)]
        ...
        for i in range(400):
            outf.write("\t".join([pc[i],pmean[i],pprop[i],pequal[i]])+"\n")
...
#            cut_weight_addup += pow( int(float(pc[i])) - float(pweight[i]) ,2)
        for i in range(400):
            outf.write("\t".join([nc[i],nmean[i],nprop[i],nequal[i]])+"\n")
...
#            cut_weight_addup += pow( int(float(nc[i])) - float(nweight[i]) ,2)
...
#    cut_weight_rmse = pow(cut_weight_addup/(peaknum*800),0.5)
...
#    outall.write("\t".join(["cut_weight_rmse",str(cut_weight_rmse)])+"\n")
    outf.close()
# ------------------------------------
# Main function
# ------------------------------------
