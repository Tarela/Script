...
def mypredict(seq,b0f,b1f,b2f,w):
    ...
    for i in range(len(seq)-w+1):
        ...
        pred.append(predY)
    return pred
def check_predict(bg0,bg2,bg8bp1,bg8bp0):
    pBG,nBG = readBG(bg0)
    p2BG,n2BG = readBG(bg2)
    pBG8bp0,nBG8bp0 = readBG(bg8bp0)
    pBG8bp1,nBG8bp1 = readBG(bg8bp1)
    code = encoding() 
    b0s0,b1s0,b2s0 = paramest(pBG)
    b0s2,b1s2,b2s2 = paramest(p2BG)

    b0s1bp8,b1s1bp8,b2s1bp8 = paramest(pBG8bp1)
    b0s0bp8,b1s0bp8,b2s0bp8 = paramest(pBG8bp0)


    # forward
    b0f = 4.0/8*b0s0 + 4.0/8*b0s2 
    ...
    # last nucleotide is counted twice so multiply by 0.5 , when checking  , don' multipy by 0.5
    b1f = numpy.vstack( (b1f, b1s2[-1,:]) )
    # concatenate dinucleotides
    b2f = numpy.vstack( (b2s0,b2s2[3:,:])     )

    outf = open('8mers0_compare_math_vs_numpy.txt','w')
    for mer8 in sorted(pBG8bp0.keys()):
        if pBG8bp0[mer8] == 0:
            realratio = -8
        else:
            realratio = numpy.log(pBG8bp0[mer8])
        predratio = predict2(mer8,b0s0bp8,b1s0bp8,b2s0bp8,8)[0]
        predratio2 = mypredict(mer8,b0s0bp8,b1s0bp8,b2s0bp8,8)[0]
        newll = [mer8,predratio,predratio2]
        outf.write("\t".join(map(str,newll))+"\n")
    outf.close()

    outf = open('6mer_compare.txt','w')
    for mer6 in sorted(pBG.keys()):
        realratio = numpy.log(pBG[mer6])
        predratio = predict2(mer6,b0s0,b1s0,b2s0,6)[0]
        newll = [mer6,realratio,predratio]
        outf.write("\t".join(map(str,newll))+"\n")
    outf.close()
   
    outf = open('8mers0_compare_direct.txt','w')
    for mer8 in sorted(pBG8bp0.keys()):
        if pBG8bp0[mer8] == 0:
            realratio = -8
        else:
            realratio = numpy.log(pBG8bp0[mer8])
        predratio = predict2(mer8,b0s0bp8,b1s0bp8,b2s0bp8,8)[0]
        newll = [mer8,realratio,predratio]
        outf.write("\t".join(map(str,newll))+"\n")
    outf.close()
    outf = open('8mers1_compare_direct.txt','w')
    for mer8 in sorted(pBG8bp1.keys()):
        if pBG8bp1[mer8] == 0:
            realratio = -8
        else:
            realratio = numpy.log(pBG8bp1[mer8])
        predratio = predict2(mer8,b0s1bp8,b1s1bp8,b2s1bp8,8)[0]
        newll = [mer8,realratio,predratio]
        outf.write("\t".join(map(str,newll))+"\n")
    outf.close()
    outf = open('8mer_compare_62combine.txt','w')
    for mer8 in sorted(pBG8bp1.keys()):
        if pBG8bp1[mer8] == 0:
            realratio = -8
        else:
            realratio = numpy.log(pBG8bp1[mer8])
        predratio = predict2(mer8,b0f,b1f,b2f,8)[0]
        newll = [mer8,realratio,predratio]
        outf.write("\t".join(map(str,newll))+"\n")
    outf.close()

   #yf  = predict2("AACTCTCATGCATCGACT"*,b0f,b1f,b2f,8)
    #yf2 = mypredict("AACTCTCATGCATCGACT"*100000,b0f,b1f,b2f,8)
...
check_predict("GM12878_ATACseq_50k_bias_onPeak_u3d3s0_newcode.txt","GM12878_ATACseq_50k_bias_onPeak_u3d3s2_newcode.txt","GM12878_ATACseq_50k_bias_onPeak_u4d4s1_newcode.txt","GM12878_ATACseq_50k_bias_onPeak_u4d4s0_newcode.txt")

def sitepro_scan(peak,out_bed,w_plus,w_minus,bg0,bg2,span,gen,lflank,rflank,offset,bpshift):
    nmer = lflank + rflank
   ...
   # print b0f,b1f,b2f
    ...
    for line in inf:### chr start end name motifscore strand FP DNase chip
        ...
        if 'N' in genome[chrm][min((start-span+1-offset +bpshift-lflank),(start-span+1 -bpshift-rflank) ):max((end+span+offset+lflank-bpshift),(end+span + bpshift + rflank))].upper():
            print line
            print 'old',genome[chrm][min((start-span+1-offset +bpshift-lflank),(start-span+1 -bpshift-rflank) ):max((end+span+offset+lflank-bpshift),(end+span + bpshift + rflank))].upper()
            continue

        if 'N' in genome[chrm][(start-11-span):(end+11+span)] : 
            print line
            print 'new',genome[chrm][(start-11-span):(end+11+span)]
            continue
        ...
        code_seq = genome[chrm][(start-11-span):(end+11+span)].upper()
        seq_f = code_seq[:-7]
        seq_r = code_seq[7:]
        
        yf  = predict2(seq_f,b0f,b1f,b2f,8)
   #     print len(seq_f),len(yf)
        yr  = predict2(revcomp(seq_r),b0f,b1f,b2f,8)[::-1]
        z   = yf + yr
        
        pnew = list(z[8:])
        nnew = list(z[:-8])

        pnew_linear = pow(numpy.e,z[8:])
        nnew_linear = pow(numpy.e,z[:-8])
       # seq_f6 = code_seq[8:-9]
...
#        pf8_assign =[]
        ...
        newll = ll + p_sum[span:(len(p_sum)-span)] + n_sum[span:(len(n_sum)-span)] + praw[span:(len(praw)-span)] + nraw[span:(len(nraw)-span)] + praw_assign + nraw_assign + px_assign + nx_assign + pnew[span:(len(n_sum)-span)] + nnew[span:(len(n_sum)-span)] + pnew_assign + nnew_assign + pnewlin_assign + nnewlin_assign
...
#========major options=============
    ...
    optparser.add_option("--w2",dest="w_minus",type="str",default = "/mnt/Storage2/home/huse/ATAC/Data/ATAC/Bam/GM12878_ATACseq_50k_m.bw",
                         help="")
    optparser.add_option("--bg0",dest="bgmatrix",type="str",default = "/mnt/Storage2/home/huse/ATAC/Data/ATAC/biasoffset/up3down3/GM12878_ATACseq_50k_bias_onPeak_u3d3s0_newcode.txt",
...
#========minor options=============
    ...
    bgmatrix2 = options.bgmatrix2
    gen = options.genome
    lflank = options.leftflank
    rflank = options.rightflank
    Cspan = options.Cspan
    offset = options.offset
    sitepro_scan(interval,out_bed,w_plus,w_minus,bgmatrix,bgmatrix2,Cspan,gen,lflank,rflank,offset,options.bpshift)
