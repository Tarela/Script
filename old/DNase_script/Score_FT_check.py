...
#            continue
        ...
        TC = sum(cut)
        C = sum(list(pcutbw.summarize(ll[0],int(ll[1]) ,int(ll[2]) ,int(ll[2])-int(ll[1])).sum_data))
        L = sum(list(pcutbw.summarize(ll[0],int(ll[1])-ml ,int(ll[1]) ,ml).sum_data))
        R = sum(list(pcutbw.summarize(ll[0],int(ll[2]) ,int(ll[2])+ml ,ml).sum_data))
...
#========minor options=============
    ...
    getsignal(inputfile,outputfile,cutbw,pspan)
