...
def cleavage_pvalue(data,matrix_p,matrix_n,span,out,prelim,cutlim):
    sdnorm = scipy.stats.distributions.norm_gen()
    zdict_p = pvalue_dict_raw(read_distribution_dict(matrix_p,prelim,cutlim))
    zdict_n = pvalue_dict_raw(read_distribution_dict(matrix_n,prelim,cutlim))
   # for i in range(5):
   #     for j in range(i+1):
   #         print i,j,zdict_p[i][j]
    ...
    for line in inf:
        ...
        for i in range(len(p_cut)):
            ...
            #sys.exit(1)
            if zdict_p.has_key(pa) and pc <= len(zdict_p[pa]) -1 : 
                #print len(zdict_p[pa]),pc
                #sys.exit(1)
                p_zscore.append(zdict_p[pa][pc])
                #print pa,pc,zdict_p[pa][pc]
            else:
                p_zscore.append(0)
            if zdict_n.has_key(na) and nc <= len(zdict_n[na]) -1 : 
        ...
        p_Pval = []
        n_Pval = []
        for i in range(len(p_zscore) - 2*span):
            P_p = combine_z_to_p(p_zscore[i:(i+2*span)] + n_zscore[i:(i+2*span)] ,sdnorm)
            #P_n = combine_z_to_p(n_zscore[i:(i+2*span)],sdnorm)
            p_Pval.append(P_p)
            #n_Pval.append(P_n)
        newll = ll + p_zscore + n_zscore +  p_Pval #+ n_Pval
        outf.write("\t".join(map(str,newll))+"\n")
    outf.close()
    inf.close()
