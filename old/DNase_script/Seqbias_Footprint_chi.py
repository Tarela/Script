        ...
        for real in range(len(dis_dict[pred])):
            if sum(dis_dict[pred]) < 10:
                ...
                if real < len(dis_dict[pred]) - 1:
                    p = (dis_dict[pred][(real)]*1.0/2+sum(dis_dict[pred][(real+1):])*1.0)/sum(dis_dict[pred])
                else:
                    p = dis_dict[pred][(real)]*1.0/(2*sum(dis_dict[pred]))
            pval = min(p,1-p) *2
            p_dict[pred].append(pval)
...
def cleavage_pvalue(data,matrix_p,matrix_n,span,out,prelim,cutlim):
    ...
    pdict_p = pvalue_dict_raw(read_distribution_dict(matrix_p,prelim,cutlim))
    pdict_n = pvalue_dict_raw(read_distribution_dict(matrix_n,prelim,cutlim))
   # for i in range(5):
   #     for j in range(i+1):
   #         print i,j,zdict_p[i][j]
    ...
    for line in inf:
        ...
        for i in range(len(p_cut)):
            ...
            #sys.exit(1)
            if pdict_p.has_key(pa) and pc <= len(pdict_p[pa]) -1 : 
                #print len(zdict_p[pa]),pc
                #sys.exit(1)
                p_pval.append(pdict_p[pa][pc])
                #print pa,pc,zdict_p[pa][pc]
            else:
                p_pval.append(1)
            if pdict_n.has_key(na) and nc <= len(pdict_n[na]) -1 : 
                n_pval.append(pdict_n[na][nc])
        ...
        Xs = []
        Ps = []
        for i in range(len(p_pval) - 2*span):
            X = combine_p_to_chi(p_pval[i:(i+2*span)] + n_pval[i:(i+2*span)] )
            P = 1-chi.cdf(math.sqrt(X),8*span)
            #P_n = combine_z_to_p(n_zscore[i:(i+2*span)],sdnorm)
            Ps.append(P)
            Xs.append(X)
            #n_Pval.append(P_n)
        newll = ll + p_pval + n_pval +  Xs + Ps #+ n_Pval
        outf.write("\t".join(map(str,newll))+"\n")
    outf.close()
    inf.close()
