...
def sitepro_scan(pattern,peak,out,w_plus,trunk,text):
    inf = open(pattern)
    ...
    for line in inf:### chr start end name motifscore strand FP DNase chip
        ...
        if chrom_len1.has_key(ll[0]):#  and chrom_len2.has_key(ll[0]):
            ...
            #    o_minus = map(int,m_sum[i:i+l])
             ...
             #           o_minus[k] = trunk
                if ll[5]=="+":
                    score =  match_pattern(p_plus,p0,o_plus,l)
                elif ll[5]=="-":
                    score =  match_pattern(m_plus,p0,o_plus,l)
                if i == len(pattern_plus_pmotif):
                    footprint.append(ll+[score,DNase])
                if last_start == "NA" :
                    last_start = i
                    last_end = i+l
                    last_value = score
...
def match_pattern(pattern_plus,pattern_plus0,observe_plus,l):
    ...
    f0=1
    for i in range(l):
        f1 = f1*Poisson_score(lambda_plus[i],observe_plus[i])/Poisson_score(lambda_plus0[i],observe_plus[i])
    #print f1,f0
...
#========major options=============
    ...
    #optparser.add_option("-c","--chipseqbw",dest="chipseqbw",type="str",
     #                    help="")
    optparser.add_option("--w1",dest="w_plus",type="str",default = "/home/sh430/Project/ChongzhiDNase/Data/bwTrack/Lncap_DHT/Lncap_DNase_50_100.bw",
                         help="")
...
#========minor options=============
   ...
   # chipseqbw = options.chipseqbw
    ...
    sitepro_scan(pattern,interval,output,w_plus,trunk,text)
