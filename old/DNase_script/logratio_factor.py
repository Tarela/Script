...
def getsignal(inputfile,outputfile,BGmatrix,pcut,ncut,lowess,pspan,tspan,gen,left,right):
    ...
    for line in inf:
        ...
        for i in range(len(pout)-2*tspan):
            ...
            ## make lowess 
            pbias_range = pbglist[i+tspan-4 : i+tspan+5 ]
            nbias_range = nbglist[i+tspan-4 : i+tspan+5 ]
            ...
            plog = math.log(float(p_ob)+1) - math.log(float(plowess) + 1)
            nlog = math.log(float(n_ob)+1) - math.log(float(nlowess) + 1)
        
            newll = [plog,nlog,strand,i,ptotal,ntotal]+pbias_range+nbias_range
