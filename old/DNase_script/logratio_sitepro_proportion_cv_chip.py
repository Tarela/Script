...
def getsignal(inputfile,outputfile,BGmatrix,pcut,ncut,conserve,chip,pspan,tspan,gen,left,right):
    pBG,nBG = readBG(BGmatrix)

    genome = twobitreader.TwoBitFile(gen)
    pcutbw = BigWigFile(open(pcut, 'rb'))
    ncutbw = BigWigFile(open(ncut, 'rb'))
    cvbw = BigWigFile(open(conserve,'rb'))
    chip_bw = BigWigFile(open(chip,'rb'))
    inf = open(inputfile)    
    ...
    for line in inf:
        ...
        cvout = list(cvbw.summarize(ll[0],max(0,int(ll[1])-pspan-tspan),int(ll[2])+ pspan + tspan,int(ll[2])-int(ll[1])+ pspan *2 + tspan*2).sum_data)
        ...
        chip_use = make_chip(chip_bw,ll,len(p_cleavage))

        newll = ll + p_cleavage +  n_cleavage + p_bias +  n_bias+  p_uniform + n_uniform + p_predict + n_predict + cv_use + chip_use
...
#========major options=============
    ...
    optparser.add_option("--right",dest="right",type="int",default=3,
                         help="")
    optparser.add_option("-c","--conserve",dest="conserve",type="str",
                         help="")
...
#    getsignal(inputfile,outputfile,bgmatrix,pcut,ncut,pspan,genome,options.left,options.right,fetch_length=100)
    ...
    getsignal(inputfile,outputfile,BGmatrix,pcut,ncut,conserve,chip,pspan,tspan,genome,options.left,options.right)
