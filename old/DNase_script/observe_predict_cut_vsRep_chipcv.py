...
def getsignal(inputfile,outputfile,BGmatrix,pcut,ncut,pcut2,ncut2,chipbw,pspan,tspan,gen,conserve):
 ...
 #       chrom_len[i['key']] = i['chromSize']
    ...
    con_bw = BigWigFile(open(conserve,'rb'))
    inf = open(inputfile)    
    ...
    for line in inf:
        ...
        for bp in range(len(pout)- 2*tspan):
            ...
            ptotal = sum(pout[bp:(bp+2*tspan)])*1.0
            ntotal = sum(nout[bp:(bp+2*tspan)])*1.0
        ...
        conout = list(con_bw.summarize(chrom,(start-pspan),(end+pspan),(end-start+2*pspan)).sum_data)
        chipout = make_chip(chip_bw,ll,len(conout))

        #print couserveout
      #  print tspan,(tspan+end-start+2*pspan),end,start,pspan
      #  print len(pobs),len(nobs),len(pobs2),len(nobs2),len(ppred),len(npred)
        newll = ll  + pobs + nobs + pobs2 + nobs2 + ppred + npred + conout + chipout  
...
#========major options=============
    ...
    optparser.add_option("-o","--outputfile",dest="outputfile",type="str",
                         help="")                         
    optparser.add_option("-b","--bgmatrix",dest="bgmatrix",type="str",default="/mnt/Storage/home/huse/Project/DNase/Data/GM12878_UWDHS/GM12878_UWDHS_r1_6mer_bg.txt",
                         help="")
...
#========minor options=============
    ...
    if not inputfile:
        ...
        sys.exit(1)
    getsignal(inputfile,outputfile,bgmatrix,pcut,ncut,pcut2,ncut2,chipbw,pspan,tspan,genome,options.conserve)
