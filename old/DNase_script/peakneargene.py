...
def read_interval(peak):
    ...
    #    strand = 1
        ...
        if not peak_dict.has_key(ll[0]):
            peak_dict[ll[0]]=[]
        peak_dict[ll[0]].append(ll)
    for chrm in peak_dict.keys():
        peak_dict[chrm].sort(key = lambda x:int(x[1]))
    return peak_dict
...
def distance(DHSdict,Expdict,out):
    outf = open(out,'w')
    print DHSdict['chr1'][:5]
    print Expdict['chr1'][:5]
    for chrm in DHSdict.keys():
        ...
        if Expdict.has_key(chrm):
            ...
            mindis="NA"
            while 1:
                Rdis = ( (int(Ds[Dindex][1])+int(Ds[Dindex][2]))/2-Es[Eindex])
                dis = abs(Rdis)
                if Rdis >= 0:
                    ...
                    if Eindex < len(Es)-1:
                        ...
                        outf.write( "\t".join(Ds[Dindex] + [str(mindis)]) + "\n")
                        if Dindex < len(Ds)-1:
                            Dindex += 1
                            mindis = "NA"
                            if Eindex > 0:
