a<-commandArgs(T)
infile <- a[1]
outfile <- paste0(a[2],'_cv_10k.pdf')
...
#    print(max(d1,d2))
    ...
    legend("topleft",c(LE,"conserv 0~0.2"),col=c(usecolor,"black"),lwd=4,bty="n")
...
par(mar=c(1.0,2,3.0,2))
bi_heatmap(pc,c("white","red"),0.99,"+Cleavage")
bi_heatmap(nc,c("white","blue"),0.99,"-Cleavage")
bi_heatmapInput(pI,c("white","red"),0.99,"+Naked")
bi_heatmapInput(nI,c("white","blue"),0.99,"-Naked")
bi_heatmap(pb,c("white","red"),0.99,"+Seqbias")
bi_heatmap(nb,c("white","blue"),0.99,"-Seqbias")
#par(mfrow=c(1,2))
