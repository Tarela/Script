...
pp <- data[order(TC),(6+4*each+1):(6+5*each)]
np <- data[order(TC),(6+5*each+1):(6+6*each)]
pz <- data[order(TC),(6+6*each+1):(6+7*each)]
nz <- data[order(TC),(6+7*each+1):(6+8*each)]

pb[pb == 0] <- 0.001
nb[nb == 0] <- 0.001
print(c(min(pc),min(pb),min(pz)))
 ### sitepro
number <- nrow(data)
rm_number <- ceiling(nrow(data)/100)

zpileup <- function(x,a,b){
	no10x <- x[which(x != 10)]
    nooutx <- no10x[order(no10x)[a:(b-a)]]
    combine_x <- sum(nooutx)/sqrt(length(nooutx))
    return(combine_x)
}
...
mpz <- apply(pz,2,zpileup,a=rm_number,b=number)
mnz <- apply(nz,2,zpileup,a=rm_number,b=number)
#mpz <- apply(pz,2,function(x,a,b) mean(x[order(x)[a:(b-a)]]),a=rm_number,b=number)
#mnz <- apply(nz,2,function(x,a,b) mean(x[order(x)[a:(b-a)]]),a=rm_number,b=number)
...
sitepro(mpz,mnz,c("red","blue"),c("plusZ","minusZ"),"Zscore")
...
bi_heatmap(pz,c("white","red"),0.99,"")
bi_heatmap(nz,c("white","blue"),0.99,"")
