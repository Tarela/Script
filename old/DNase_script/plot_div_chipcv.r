...
each<-(ncol(data)-6)/8
TC <- apply(data[,(6+7*each+1):(6+8*each)],1,mean)
#length(which(TC > 0.5))
...
pc <- as.matrix(data[oc,(6+1):(6+each)])+0.05
nc <- as.matrix(data[oc,(6+each+1):(6+each+each)])+0.05
allc <- pc+nc
pc2 <- as.matrix(data[oc,(6+each+each+1):(6+each+each+each)])+0.05
nc2 <- as.matrix(data[oc,(6+3*each+1):(6+4*each)])+0.05
allc2 <- pc2+nc2
pp <- as.matrix(data[oc,(6+4*each+1):(6+5*each)])+0.05
np <- as.matrix(data[oc,(6+5*each+1):(6+6*each)])+0.05
allp <- pp+np
conv <- as.matrix(data[oc,(6+6*each+1):(6+7*each)])
chip <- as.matrix(data[oc,(6+7*each+1):(6+8*each)])
...
mconv <- apply(conv,2,function(x,a,b) mean(x[order(x)[a:(b-a)]]),a=rm_number,b=number)
...
pdf(file=outfile,height=6,width=15)
...
layout(matrix(c(1,2,20,3,21,4,5,seq(6,18,2),seq(7,19,2)),nrow=3,byrow=T),heights=c(1,2,0.25),width=c(1,1,0.2,1,0.2,1,1))
sitepro1(mallc,"red","","observe cleavage")
sitepro1(mallcvp,"blue","","abs(log2(obs/exp)),mean")
sitepro1(mallcvc,"blue","","abs(log2(rep1/rep2)),mean")
sitepro1(mchip,"brown","","chip")
sitepro1(mconv,"black","","phasCon")
...
bi_heatmap(chip,c("white","brown"),0.99,"")
bi_heatmap(conv,c("white","black"),0.99,"")
