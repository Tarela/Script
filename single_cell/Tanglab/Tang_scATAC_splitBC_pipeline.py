...
def splitATACfq(inputname,metafile,folder,P5,P7,startpos):
    ...
    for line in inf:
        ...
        if bc2cell.has_key(BC):
            ...
            bc2cell[BC] = [open(ll[5]+"_R1.fastq",'w'),open(ll[5]+"_R2.fastq",'w')]
            bc2cellEX[BC] = [open(ll[5]+"ex_R1.fastq",'w'),open(ll[5]+"ex_R2.fastq",'w')]
            bc2cellname[BC] = ll[5]
    inf.close()

    unmatchcell = [open(inputname+"_unmatch_R1.fastq",'w'),open(inputname+"_unmatch_R2.fastq",'w')]

    ## scan fastq, fetch barcode count list
    if not folder.endswith("/"):
        folder += "/"

    fq1 = "%s%s_R1.fastq"%(folder,inputname)
    fq2 = "%s%s_R2.fastq"%(folder,inputname)
    inf1 = open(fq1,"r")
    inf2 = open(fq2,"r")
